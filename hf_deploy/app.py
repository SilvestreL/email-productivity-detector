...
# Importar o carregador do Google Drive para HF
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        # Seção: Configuração do Google Drive (apenas para HF)
        st.markdown('<div class="sb-section">', unsafe_allow_html=True)
        st.markdown("<h4>Configuração</h4>", unsafe_allow_html=True)
        
        # Botão para configurar IDs do Drive
        if st.button("🔧 Configurar Google Drive", key="drive_config_btn"):
...
def get_classifier():
    ...
    try:
        # Carregar modelo do Google Drive (para HF Spaces)
        ...
        st.info(
            "Certifique-se de que o modelo está disponível no Google Drive"
        )
        return None
...
def main():
    ...
    # Configuração do Google Drive (apenas para HF Spaces)
